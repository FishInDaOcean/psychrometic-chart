...
# -----------------------------------------------------------------------------
# Thermodynamic Equations (ASHRAE / CIBSE Standards)
# -----------------------------------------------------------------------------
P_ATM = 101.325  # kPa
...
# Sidebar & Inputs
# -----------------------------------------------------------------------------
st.sidebar.title("📐 Exam Question 1 Solver")
...
oa_to_ra_ratio = st.sidebar.number_input("OA : RA Ratio (1 : X)", value=3.0, step=0.5)

st.sidebar.markdown("---")
st.sidebar.subheader("Part 2: Reheat Modification")
rshf_new = st.sidebar.slider("New RSHF (after usage change)", 0.3, 1.0, 0.60, 0.05)

# -----------------------------------------------------------------------------
# Solver Engine (Parts a - h)
# -----------------------------------------------------------------------------
w_r = calc_w(t_r, rh_r)
h_r = calc_h(t_r, w_r)
...
# a) Mixed Air (M)
t_m = (oa_fraction * t_o) + ((1.0 - oa_fraction) * t_r)
w_m = (oa_fraction * w_o) + ((1.0 - oa_fraction) * w_r)
h_m = calc_h(t_m, w_m)

# b) Off-Coil Supply Point (S)
...
for t in range(-5, 46, 5):
    fig.add_trace(go.Scatter(
        ...
        mode="lines", line=dict(color=GRID_COLOR, width=0.7 if t % 10 != 0 else 1.2),
        hoverinfo="skip", showlegend=False
    ))

# Moisture Content grid lines
for w_val in np.arange(0.001, 0.026, 0.001):
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines", line=dict(color=GRID_COLOR, width=0.6 if round(w_val * 1000) % 5 != 0 else 1.1),
        hoverinfo="skip", showlegend=False
    ))

# Percentage Saturation curves
...
for rh in range(10, 101, 10):
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines", line=dict(color=SAT_COLOR if rh == 100 else GRID_COLOR, width=2.5 if rh == 100 else 1.0),
...
fig.add_trace(go.Scatter(
    ...
    line=dict(color="#d97706", width=2.5), name="Mixing Line (O-R)"
))

# 2. Cooling Coil Line M - ADP
fig.add_trace(go.Scatter(
    x=[t_m, t_adp], y=[w_m, w_adp], mode="lines",
    line=dict(color="#0284c7", width=2.5), name="Cooling Line (M-ADP)"
))

# 3. Initial Room Process Line S -> R (RSHF = 0.75)
fig.add_trace(go.Scatter(
    x=[t_s, t_r], y=[w_s, w_r], mode="lines",
    line=dict(color="#dc2626", width=2.5, dash="dash"), name=f"Room Line 1 (RSHF={rshf_orig})"
))
...
# 5. Modified Room Process Line S' -> R (RSHF = 0.60)
fig.add_trace(go.Scatter(
    x=[t_s_prime, t_r], y=[w_s_prime, w_r], mode="lines",
    line=dict(color="#ea580c", width=2.5, dash="dot"), name=f"Room Line 2 (RSHF={rshf_new})"
))

# State Node Markers
points = [
    ("O (Outdoor)", t_o, w_o, "#dc2626"),
    ("R (Room)", t_r, w_r, "#2563eb"),
    ("M (Mixed)", t_m, w_m, "#d97706"),
    ("ADP", t_adp, w_adp, "#059669"),
    ("S (Off-Coil)", t_s, w_s, "#0284c7"),
    ("S' (Reheat Supply)", t_s_prime, w_s_prime, "#9333ea")
]

for label, px, py, pcol in points:
    fig.add_trace(go.Scatter(
        x=[px], y=[py], mode="markers+text",
        marker=dict(size=12, color=pcol, line=dict(color="#ffffff", width=1.5)),
        text=[label.split()[0]], textposition="top right",
        name=label, hoverinfo="text",
        hovertext=f"<b>{label}</b><br>Tdb: {px:.2f}°C<br>W: {py*1000:.2f} g/kg"
    ))
...
c3.metric("c) Supply Volume Flow ($\dot{V}_S$)", f"{v_dot_s:.2f} m³/s", f"v_S = {v_s:.3f} m³/kg")

c4, c5, c6 = st.columns(3)
c4.metric("d) Coil Bypass Factor ($BF$)", f"{bf:.3f}", f"{(bf*100):.1f}%")
c5.metric("e) Cooling Coil Load ($\dot{Q}_{coil}$)", f"{q_coil:.2f} kW")
c6.metric("f) Room Total Heat Load ($\dot{Q}_{room}$)", f"{q_room:.2f} kW")

c7, c8, c9 = st.columns(3)
c7.metric("g) New Supply Tdb ($T_{S'}$)", f"{t_s_prime:.2f} °C", f"Reheat $\Delta T$ = {(t_s_prime - t_s):.2f} °C")
c8.metric("h) New Supply Mass Flow ($\dot{m}_{S, new}$)", f"{m_s_new:.3f} kg/s", f"$\Delta \dot{m}$ = +{(m_s_new - m_s):.2f} kg/s")
